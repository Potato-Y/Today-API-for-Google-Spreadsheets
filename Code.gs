var SHEET_NAME="Main Sheet"; //시트 이름 리스트ar SHEET_NAME=["Main Sheet"]; //시트 이름 리스트
...
function handleResponse(e){
  ...
  try{
    ...
    if(sheetCount<2){ //log를 저장할 곳이 없다면
      addSheet();
    ...
    return ContentService.createTextOutput(JSON.stringify({"result":"success","total":totalRange.getValue(),"today":todayRange.getValue()}))
           .setMimeType(ContentService.MimeType.JSON);
  }catch(e){
    Logger.log("Error: "+e);
    return ContentService.createTextOutput(JSON.stringify({"result":"error","msg":e}))
           .setMimeType(ContentService.MimeType.JSON);
  }finally{
    lock.releaseLock();
  }
